...
VERSIONSC () {
    Andre-Sakti=V3.0
    IZINVERSION=$(curl https://raw.githubusercontent.com/andre-sakti/kintil/main/ip | grep $MYIP | awk '{print $6}')
    if [ $Andre-Sakti = $IZINVERSION ]; then
...
VALIDITY () {
    ...
    echo -e "\e[31mTolong Renew Script di  @Andresakti\e[0m"
    exit 0
fi
}
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/andre-sakti/mokondo/main/"
...
print_install "Membuat direktori xray"
    ...
    mkdir -p /var/lib/Andre-Sakti >/dev/null 2>&1
    # // Ram Information
...
# // Installing UDP Mini
mkdir -p /usr/local/Andre-Sakti/
wget -q -O /usr/local/Andre-Sakti/udp-mini "${REPO}badvpn/udp-mini"
chmod +x /usr/local/Andre-Sakti/udp-mini
...
clear
# banner
echo "Banner /etc/Andre-Sakti.txt" >>/etc/ssh/sshd_config
sed -i 's@DROPBEAR_BANNER=""@DROPBEAR_BANNER="/etc/Andre-Sakti.txt"@g' /etc/default/dropbear

# Ganti Banner
wget -O /etc/Andre-Sakti.txt "${REPO}ssh/issue.net"
